...
def optimize_solution(org_solution, prob, cargo_dict, feas_memo, temperature, a, iterations=200, print_iter=False):
	...
	while iter < iterations:
		...
		scaler = randint(1, n_vehicles // 10 + 3)
...
def warmup_solution(org_solution, prob, cargo_dict, feas_memo, iterations=100):
	...
	for _ in range(iterations):
		...
		feasibility, new_sol = insert_number_at_all_positions_pick_first(incumbent_sol1, move_to, move_from, cargo_to_move, prob, feas_memo)
...
def run_tests(num_tests, iterations, warmup, prob, print_iter=False, feasability_memo=set()):
	...
	for test in range(num_tests):
		...
		delta_avg = warmup_solution(incumbent, prob, cargo_dict, feasability_memo, iterations=warmup_iterations)
		if not delta_avg:
			temperature = 100000
			a = 0.999
